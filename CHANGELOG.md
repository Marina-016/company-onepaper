...
- **修复 §2.2 千分位数被切碎（`_build_operating_fact_cards`）**：事实卡数值分词器 `(?<!\d)([-+]?\d+(?:\.\d+)?)` 会从千分位中间切开数字——`超过200,000家线下零售药店` 被切出残片 `000家`，渲染成 `…零售药店000家[49]`。数字改为容纳千分位并用 `(?<!\d,)` 排除尾段，同步修复 `_QUANTITATIVE_TOKEN_RE`（溯源校验用同一缺陷分词器），`measures` 取值时去掉千分位。该缺陷还会让 `200,000家` 被误判成 `000家` 再与来源比对，造成 `经营数字无来源数值` 误报。
- **修复单位交替丢量纲（同函数）**：`unit_re` 把 `万` 排在 `万家` 之前，`25万家零售药店` 命中成 `25万`、量纲丢失。单位表改为“先长后短”，补 `万家/万人/万户`。§9.4 核心变量不再出现 `25万[11]` 这类无法判读基准的裸数字。
- **修复标记就地贴引产生悬空数字（`_render_fact_markers`）**：模型把 `{{FACT:F11}}` 落在小句末尾、而该数值已在小句中陈述时，会渲染成 `…DTP药房等渠道25万家[11]`。新增同句去重——该小句已含此数值即丢弃标记，交由既有死引用清理回收来源，绝不把引用贴到错误位置。
- **修复风险标题词中截断（`_truncate_risk_title` / `_derive_a_share_risk_title`）**：超出 24 字硬上限时，标点回退全部落空便直接 `cut`，产出 `…每年降约15%的收`、`…每年下滑15%的缺口风` 等半截词。现优先去掉可省助词“的”以保住结尾风险名词，其余按语义边界回退；`_derive_a_share_risk_title` 的 `candidate[:24]` 硬切也统一改走该函数。
- **补 §10 主路径标题结构门禁（`_validate_render_section_10`）**：fallback 路径一直调用 `_is_valid_a_share_risk_title`，主路径此前完全不校验标题结构，模型自行硬切出的 `瑞普泊肽等GLP-1管线2027年上市与临床不确` 得以进入交付物。新增 `_has_dangling_title_tail` 判据——虚词尾（的/与/和/未…）、“的+单汉字”尾、以及**标题末两字在整个证据池中无从查证**即判为硬切。刻意不套用 `_is_valid_a_share_risk_title` 的关键词白名单：该白名单面向 fallback 的碎片化标题提炼，会误杀 `仿制药2026-2028年收入年降约15%` 这类合法但未含风险词的标题。
- **修复 §9.4 基准值标签错配（事实卡下发指标名）**：提示块此前只给「数值+原文片段」，模型在 `毛利率持平于85%……而销售与营销费用率从33%降至32%` 这类同句多指标片段上会把 85% 错标成费用率；而 `_infer_operating_fact_label` 实际已算出「毛利率」，只是没下发。事实卡增加 `指标:` 字段并绑定 prompt 约束。同时补 `零售终端覆盖/医院覆盖` 标签、`毛利率` 判定、`产品价格降幅` 判定，修正 §9.4 基准值退化为裸数字（`85%`、`25万`）而无指标名的问题。

### §1 近况事件日期倾向

- §1 提示词新增“事件日期倾向”：近况条目若描述已发生的具体事件（获批/受理/签约/到账/临床读出/业绩发布/政策落地等），正文尽可能带上事件日期，便于读者判断新鲜度；标题仍以“事件+数字+判断”为主。**仅为写作倾向、不加入校验**：来源未给出日期、或条目为持续状态/业务方向时，不得为凑日期而臆造或用模糊措辞替代。

### 重复堆叠代码清理

- **`gen_maincomp_fallback` 指标抽取收敛为规格表**：金融股营收构成的抽取逻辑（逐行 `re.search` + 首 match 优先）此前按指标复制了 11 遍，且取值键与下方表格行两处各自声明、增删指标时易不同步。新增模块级 `_FIN_REVENUE_METRIC_SPECS` 统一承载「取值键 / 数值正则 / 同比键 / 同比正则」，抽取改为单层循环。已对 HEAD 版本做 7 组用例（含千分位、负同比、同指标重复、缺值剔除、转义换行、边界年数）逐一比对，输出完全一致。
- 扫描结论：全仓库无函数体完全相同的重复；`gen_reference_section` 的 `elif ref_type == …` 分支虽形态相近，但每个来源类型的输出字段与 API 名互不相同，属格式分派，不构成可合并的堆叠，保持原样。

### 版本号统一

- 版本号统一为 `v1.3`（`SKILL.md` frontmatter、代码内 TODO 标注、测试文件与产物目录）。历史溯源标注（如 `v1.2.49` 规则引入记录）保持原值不变。
- 回归测试更名 `tests/test_v13_provenance_fixes.py`，扩充至 18 例，覆盖上述根因的真阳性/真阴性边界、事实卡指标名下发、以及 `gen_maincomp_fallback` 的抽取与首 match 语义。

## v1.2.49

- **§1 近况小标题加粗**：`_normalize_section1_recent_format` 不再整体去除 §1 bullet 加粗，改为统一规范化——无论模型输出整行加粗、未加粗还是已加粗小标题，最终还原为 `• **事件+判断**：事实…[N]`，冒号前小标题加粗、正文不加粗。
- **§2 长短期逻辑具体化**：`gen_sections_1_2_3` 的 §2 prompt 重写为「变化→预期差→验证点」三段式——每条必须写清事件主体与来源数字、市场预期差落在收入/利润/估值哪个科目、可观察的验证信号与方向性影响；新增【当前估值锚】注入 PE/PB 与一致预期，禁止“有望/或将/未来可期”空泛收尾和自造倍数、目标价。
- **§1/§2 标题数字锚点化（对标同业竞品）**：参考竞品一页纸（宁德时代_一页纸.docx），§1 近况标题要求嵌入最有信息量的数字（金额/规模/占比/增速/份额/产能/排产），形成“事件+数字+判断”（如“200-400亿注销式回购落地”）；§2 长短期标题=带观点与证据的投资判断，鼓励嵌数字锚点并点出预期差锐度（“市场预期过于悲观”“利空出尽”“尚未定价”），如“全球动储需求增速20%+，市场预期过于悲观”。示例数字仅供示范标题结构，严禁照搬、必须用该公司真实数字。
- **同业表进展列信任 LLM 优先 + 判定式兜底**：`_validate_peer_table` 重构——peer 有自身材料时，LLM 单元格若**含自有合法引用且正文是真实事件句**（判定：非“围绕XX等业务方向推进产品迭代与应用拓展”泛化方向句）直接保留；仅当 LLM 漏写引用、写错引用（如引标的研报）或写泛化方向句时，才用 `_derive_peer_progress` 确定性提炼真实业务事件并附自有引用。修复了此前 Branch A 因错引用得到“—”退化成泛化句、有材料 peer（豪鹏、国轩）进展列无真实事件的问题，同时避免 LLM 高质量输出被规则覆盖。`_derive_peer_progress` 标题路径放宽（事件词表从硬过滤降为评分加分），并新增 `_is_passive_peer_mention` 守卫剔除把 peer 当作他人客户/供应商/名单成员被动提及的句子。
- **§10 风险标题具体化**：`gen_section10` 要求标题写清“具体对象+变化”并尽量嵌入关键数字（如“存货1308亿元同比+81%的备货去化风险”），禁止“库存风险/海外风险/产能风险”等抽象类别标题；标题长度上限由 20 放宽至 24 字（与校验器一致）。
- **§10 校验韧性化**：`_validate_render_section_10` 对超长标题就地截断（`_truncate_risk_title` 优先在标点断句）；单条风险不合格（标题过长/泛化/重复主题/缺引用）只跳过该条，仅当有效风险不足 5 条才整体失败——此前任一 title_len 超限都会打回整段，fallback 无证据时直接阻断整篇报告。
- **§1-3 token 上限提升**：`gen_sections_1_2_3` 的 `max_tokens` 3800→4800，为 §2 三段式具体内容留出输出空间。
- **§2 裸段落补 bullet**：新增 `_normalize_section2_bullets`，把 2.1/2.2 下失去标记的中文裸段落补回 `• ` 列表项，保证每条要点以列表呈现。
- **H2 全局去重**：`_dedup_section_titles` 的 H2 去重由“相邻两行内”放宽为全局（同标准化标题只保留第一个），消除 LLM 章节内容偶发嵌入同类 H2（如 §7 自带“## 7 公司调研大纲”）导致自检“重复H2”阻断整篇。
- 同步更新 `references/a-share-report-structure.md` 与 `references/a-share-quality-checklist.md` 的对应写作规范与质量清单。

## v1.2.48

- §1 固定改为“近况 3–5 条 → 机构观点与估值 → 市场一致预期”的买方快读格式；近况统一使用“**具体事件 + 判断**：事实、数字与克制市场含义[N]”，最近两日个股资讯优先进入近况而非独立监控块。
- §10 风险提示由 3–4 条扩展至 5–7 条；证据池新增近两日 `stockSentimentNews`、管理层讨论和近期公告，并保留研报、纪要及调研。公告新增参考资料条目和“引用→原文”解析，资讯型风险可完成溯源核验。
## v1.2.47

- **§1 近况跟踪去重**：移除组装层的 `_render_s1_monitoring_block` 独立注入（`s1_monitoring` 置空），改为在 `gen_sections_1_2_3` 的 Prompt 中明确指示 LLM 将资讯直接整合进 §1 要点。新增"⚠️ 组装层不会再叠加任何监控块"铁律，从根源消除资讯事件重复出现。
- **同业表 Branch B 修正**：`_validate_peer_table` peer 循环中，当 peer 有自身材料（`required_refs` 非空）但 LLM 未写引用时，不再退化为 `_compact_peer_profile_progress`（无引用泛化方向句），改为强制走 `_derive_peer_progress` 从自身材料重新提炼并附引用；无材料的 peer 才允许保留泛化业务方向。

## v1.2.46

- A 股 §1 近况事件源由不稳定的 `Stock_Monitoring_Events` 替换为 `stockSentimentNews`：采集最近 4 个自然日资讯，保留标题、摘要、来源、生效时间、情绪与 URL；按精确时间排序，writer 仅使用最新两个事件日期。情绪标签仅作排序线索，不直接作为投资结论。

## v1.2.43

- §10 风险提示 prompt 改为买方视角的自主风险判断：按未来6–12个月对业绩、估值和市场预期的影响排序，优先近期且有明确经营传导链的事项；不机械复述研报末尾风险提示。
- 保留来源、引用、3–4条及长度门禁；新增公司特有标题、具体传导路径与跟踪项的写作指引。客户自研、份额变化、订单调整、产能延期、库存/减值及政策变化仅作为材料内的待判断事项，不强制覆盖。
## v1.2.42

- A 股新增 `Stock_Monitoring_Events` 近 4 个自然日市场监控事件采集，并与公告/研报/会议并行执行。§1 只读取最新两个事件日期，最新事件优先写入首条；若模型遗漏，由 writer 以原始标题和引用补入，避免近况回退为半年财报摘要。
- 市场异动归因限定为来源已明示的事实；因果不足时仅表述“市场关注/市场担忧”。研报和公告统一按发布日期倒序排列，消除并发返回顺序导致的旧材料抢占。

## v1.2.41

- §8.2 的“相关业务进展”改为双层降级：有该 peer 自身材料时仍引用其业务事件；完全无自身材料时，允许仅基于已确认的可比业务、商业模式和核心产品生成一句不含数字或具体事项的克制业务方向，不再整格留空。统一移除“事项：/事件：/进展：”前缀，避免表格出现公文式表达。
- 恢复 DOCX 参考资料的 10.5pt 正文字号与正文色，保留编号加粗和悬挂缩进；撤销 v1.2.40 的 8.5pt 浅灰小字样式，保证阅读与打印可读性。
- §9.4 确定性事实卡排除定增/发行价/融资/回购/股价等资本市场数字以及近100%的报表覆盖类收入占比；预付款项、存货等营运资本变量单列为供应链备货维度，三档情景按变量类型写经营与估值传导，避免把融资价格误写为产品价格、把预付款项误写为产能。
- 清理无编号 `{{FACT:}}` 占位符泄漏，并新增同业前缀、无材料业务方向、事实占位符等回归用例。
## v1.2.40
