...
- 采集端使用 `Stock_Monitoring_Events` 拉取近 4 个自然日事件；写作端只向 §1 暴露最新两个事件日期，以覆盖最近 1–2 个交易日且避免周末漏数。
- 一旦有可引用的市场监控事件，§1 第一条必须优先呈现该事件；对涨跌归因仅陈述来源已明示的事实，因果不充分时使用“市场关注/市场担忧”，不得擅自写成唯一原因。
- 市场监控事件独立于公司公告、研报和同业材料，参考资料标记 API：`Stock_Monitoring_Events`；无事件时才退回近一个月研报、公告和会议材料。
