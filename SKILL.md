...
- 标的行优先使用模型生成的自身材料进展；模型漏填或写成纯财务摘要时，writer 仅可从同一批标的研报提取一条 20–60 字、带引用的完整业务句兜底。仍无可审计进展时才整体删除“相关业务进展”列。
...
- 三档只描述相对当前基准的经营方向及收入、利润、现金流传导；估值含义只写估值中枢、风险溢价和市场预期的方向性变化，不输出公式、价格或说明性话术。
...
- 风险提示只使用目标公司的真实材料，输出 3–4 条“触发条件 → 影响路径 → 跟踪项”并附来源。若来源已有与风险直接相关的经营或财务指标，优先保留 1–2 条量化基线；没有直接证据时才用定性跟踪，不得补造阈值、客户或订单。
