...
- **🔴 只能按名称搜，不能按6位代码搜**：用代码当 `query` 会被当成语义文本，返回数字谐音的无关结果（实测 `000848`→三六零、`600519`→二六三，且不同代码常返回**相同**垃圾）。需要代码反查名称时，改走 `Ashare_info`（取 `companyName`）再用全称反查本接口回收简称——`scripts/fetch_data.py --resolve-only` 已封装该逻辑，**优先用脚本，不要手搓**。
