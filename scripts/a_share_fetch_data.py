...
def fetch_fin_indicators(meta, ticker, token):
    ...
    if not url:
        return None, "URL缺失"
    rj, _, err = call("GET", url, token, params={"ticker": ticker, "period": "3"})
    if err:
        return None, err
    if not _api_ok(rj):
        return None, f"API返回失败: code={rj.get('code')}, msg={rj.get('message','')[:80]}"
...
def fetch_research_reports(meta, ticker, company_name, token,
    ...
    # 外资机构类型标识（orgType 字段可能的取值）
    FOREIGN_ORG_TYPES = {"外资", "境外", "FOREIGN", "foreign", "Foreign"}

    domestic_ids, foreign_ids = [], []
    for it in priority:
        try:
            rid_int = int(it["_id"])
            org_type = str(it["_meta"].get("orgType") or "")
            if org_type in FOREIGN_ORG_TYPES:
                foreign_ids.append(rid_int)
            else:
                domestic_ids.append(rid_int)
        except Exception:
            pass
    ...
    # 合并两个批量结果的 data 字段
    merged_data = {}
    for r in [dom_result, for_result]:
        if r and isinstance(r, dict):
            sub = r.get("data") or {}
            if isinstance(sub, dict):
                merged_data.update(sub)
    if merged_data:
